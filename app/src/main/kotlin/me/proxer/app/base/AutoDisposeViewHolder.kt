...
abstract class AutoDisposeViewHolder(
    itemView: View
) : RecyclerView.ViewHolder(itemView), LifecycleScopeProvider<ViewHolderEvent> {

    private val lifecycleEvents by lazy { BehaviorSubject.create<ViewHolderEvent>() }

    fun onBind() = lifecycleEvents.onNext(BIND)

    fun onUnbind() = lifecycleEvents.onNext(UNBIND)
